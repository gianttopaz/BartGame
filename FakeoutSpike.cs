...
namespace BartGame
{
    class FakeoutSpike : Enemy
    {
        ...
        public FakeoutSpike(int x, int y, Player player)
        {
            name = "spike";
        ...
        public override void Update(GameTime gameTime, List<Sprite> sprites)
        {
            ...
            if (playerPosition.Right > position.X && playerPosition.Left < (position.X + 16))
            {
                if (!passing)
                    primed++;
                passing = true;
                if (primed == Primed.Triggered)
                    state = State.Falling;
            }
            ...
            {
                if ((playerPosition.Right > (position.X - 48) && (playerPosition.Right < position.X)) 
                    || (playerPosition.Left > (position.X + 16) && playerPosition.Left < (position.X + 48) && !passing))
                {
